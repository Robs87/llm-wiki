...
WIKI_DIR = Path(os.environ.get("WIKI_PATH", os.path.expanduser("~/Desktop/wiki")))
GRAPH_DIR = WIKI_DIR / "graph"
REPO_ROOT = WIKI_DIR  # for relative path display
...
def append_log(entry: str):
    ...
    if not log_path.exists():
        log_path.write_text(
            "# Wiki Log\n\n"
            "> Records important additions, revisions, and clarifications in the project knowledge layer. Newest entries first (reverse chronological).\n\n"
            f"{entry_text}\n",
            encoding="utf-8",
        )
        return
